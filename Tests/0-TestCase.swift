...
class TestCase : XCTestCase {
	...
	func verifyErrorThrown<T>(in file: StaticString = #file, at line: UInt = #line, from closure: (Error) async throws -> T) async {
...
extension TestCase {
	...
	class Collector {
		...
		func tryCollect(_ value: Int, throwError error: Error? = nil) async throws {
		...
		func tryCollectAndTransform(_ value: Int, throwError error: Error? = nil) async throws -> String {
			try await tryCollect(value, throwError: error)
			return String(value)
		}
		
		func tryCollectAndDuplicate(_ value: Int, throwError error: Error? = nil) async throws -> [Int] {
			try await tryCollect(value, throwError: error)
			return [value, value]
		}
	}
