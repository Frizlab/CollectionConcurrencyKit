...
/* These tests are disabled unless the 'TIMING_TESTS' compiler flag has been defined, since they involve delays.
 * To run these tests, use `swift test -Xswiftc -DTIMING_TESTS`. */
#if TIMING_TESTS
...
private extension TimingTests {
	...
	func verifyExecutionTime<T>(
		...
		runAsyncTest{ array, _ in
			...
			switch expectation {
				case .lessThan   (let time): XCTAssertLessThanOrEqual   (executionTime, time, file: file, line: line)
				case .greaterThan(let time): XCTAssertGreaterThanOrEqual(executionTime, time, file: file, line: line)
			}
		}
	}
